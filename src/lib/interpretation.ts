...
function buildDayPhrase(fragments: string[]): string {
  const anchor = fragments[1] ?? fragments[0];
...
function buildHiddenValues(fragments: string[]): HiddenValue[] {
  const values: HiddenValue[] = fragments.slice(0, 3).map((fragment, index) => {
    const templates = [
      `제가 보기에는 "${clip(fragment, 18)}" 속에 스스로를 놓치지 않으려는 마음이 숨어 있는 것 같아요.`,
      `"${clip(fragment, 18)}"라는 장면에서 오늘을 세밀하게 바라보는 힘이 보여요.`,
      `이 기록에는 "${clip(fragment, 18)}"을 지나온 사람만 알 수 있는 작은 배움이 느껴져요.`,
    ];

    return {
      id: createId("value"),
      text: templates[index % templates.length],
      evidence: clip(fragment, 42),
    };
  });

  while (values.length < 3) {
    const fallback = fallbackValues[values.length];
    values.push({
      id: createId("value"),
      text: fallback,
    });
